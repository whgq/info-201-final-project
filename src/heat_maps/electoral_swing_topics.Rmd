...
We first match cities from Google trends to electoral data. We do so by normalizing the city names in the Google tends dataset.

```{r}
interest_swing_08_12 <- change_time_period("08-12", "04-08") %>% mutate(city = toupper(City)) %>% select(city, change_query_incidence)
swing_county <- votestimeframe %>%
    group_by(state, county_name, office) %>%
    summarise(swing = .data[["2012"]] - .data[["2008"]]) %>%
    mutate(city = county_name) %>%
    select(city, swing)

merge(interest_swing_08_12, swing_county, by = "city") %>% head(10)
```
...
To generate a heatmap, we get latitutde and longitudes for each city, then plot the values.
...
with_lat_long <- merge(interest_swing_08_12, swing_county, by = "city") %>%
    (\(df) df %>% mutate(lat_long = geocode(output = "latlon", location = df$city, method = "census")))

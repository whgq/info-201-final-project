...
To generate a heatmap, we get latitutde and longitudes for each county, then plot the values. We save this dataframe for convenience. To speed up knitting, we use our cached .csv file.
...
with_lat_long %>% filter(!is.na(change_query_incidence)) %>% head(10)
```

Looks good.
Now, we generate a bivariate heatmap. We followed this guide to generate the map: [Bivarite Mapping with ggplot2](https://cran.r-project.org/web/packages/biscale/vignettes/biscale.html).
...
classified <- with_lat_long %>%
    bi_class(x = change_query_incidence, y = swing, style = "quantile", dim = 2) %>%
    mutate("long" = lat_long.lon, "lat" = lat_long.lat)

ggplot(data = map_data("state") %>% filter(long > -140)) +
  geom_polygon(aes(x = long, y = lat, group = group), color = "white") +
  geom_point(data = classified %>% filter(long > -140), aes(x = long, y = lat, color = bi_class), alpha = 1) +
  bi_scale_fill(dim = 2) +
  labs(
    title = "Change in Search Interest for \"Immigration\" vs Electoral Swing from 2004 - 2012",
  ) +
  bi_theme(legend.position = "none", title = element_text(size = 8))
```

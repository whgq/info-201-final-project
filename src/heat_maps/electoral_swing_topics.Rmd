...
## 5. Heatmap: Query Incidence Swing vs Electoral Swing (2008 election to 2012 election)
...
swing_county <- votestimeframe %>%
    group_by(state, county_name, office) %>%
    summarise(swing = .data[["2012"]] - .data[["2008"]]) %>%
...
To generate a heatmap, we get latitutde and longitudes for each city, then plot the values. We save this dataframe for convenience. To speed up knitting, we use our cached .csv file.
...
#with_lat_long <- merge(interest_swing_08_12, swing_county, by = "city") %>%
#    (\(df) df %>% mutate(lat_long = geocode(output = "latlon", location = paste(df$city, ", ", df$state), method = "census")))

#with_lat_long %>% write.csv("../../data_cleaned/swing_interest_vs_electoral_08_12_lat_long.csv")
#with_lat_long %>% head(10)

with_lat_long <- read.csv("../../data_cleaned/swing_interest_vs_electoral_08_12_lat_long.csv")

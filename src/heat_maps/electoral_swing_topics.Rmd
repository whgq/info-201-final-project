...
plot <- ggplot(data = map_data("state") %>% filter(long > -140)) +
  ...
  geom_point(data = with_corr %>% filter(long > -140), aes(x = long, y = lat, color = as.numeric(local_slope))) +
  scale_colour_gradient2(low = "blue", high = "red", mid = "gray", midpoint = 0, limits=c(-0.05, 0.075)) +
  labs(
...
plot <- ggplot(data = map_data("state") %>% filter(long > -140)) +
  geom_polygon(aes(x = long, y = lat, group = group), color = "white") +
  geom_point(data = with_corr.2 %>% filter(long > -140), aes(x = long, y = lat, color = as.numeric(local_slope))) +
  scale_colour_gradient2(low = "blue", high = "red", mid = "gray", midpoint = 0, limits=c(-0.05, 0.075)) +
  labs(

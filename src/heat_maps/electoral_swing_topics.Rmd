...
votestimeframe <- electiondf %>%
  ...
  summarise(year, state, county_name, margin = (candidatevotes / totalvotes) * 100) %>%
  ungroup() %>%
  filter(party == "REPUBLICAN") %>%
  distinct(state, county_name, year, .keep_all = TRUE) %>%
  select(state, county_name, year, margin) %>%
  pivot_wider(id_cols = c(state, county_name), names_from = year, values_from = c(margin))
...
swing_county <- votestimeframe %>%
    group_by(state, county_name) %>%
    summarise(swing = .data[["2012"]] - .data[["2004"]]) %>%
...
plot <- ggplot(data = map_data("state") %>% filter(long > -140)) +
  ...
  labs(
    ...
    color = "Swing in Election Results (Red = More Republican)",
...
swing_county <- votestimeframe %>%
    group_by(state, county_name) %>%
    summarise(swing = .data[["2016"]] - .data[["2008"]]) %>%
    mutate(county = county_name) %>%
    select(county, swing)

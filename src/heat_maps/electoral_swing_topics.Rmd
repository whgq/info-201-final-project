...
votestimeframe %>%
    group_by(state, county_name, office) %>%
    summarise(swing = (.data[["candidatevotes_2008"]] / .data[["totalvotes_2008"]]) - (.data[["candidatevotes_2004"]] / .data[["totalvotes_2004"]])) %>%
    head(10)
```
...
swing_county <- votestimeframe %>%
    group_by(state, county_name, office) %>%
    summarise(swing = (.data[["candidatevotes_2012"]] / .data[["totalvotes_2004"]]) - (.data[["candidatevotes_2004"]] / .data[["totalvotes_2004"]])) %>%
...
To generate a heatmap, we get latitutde and longitudes for each county, then plot the slope of the linear regression at the 10 geographically nearest neighbors at a given pair of query_incidence and swing.
...
get_local_slope <- function(df, row_number, n = 5) {
  # Assumes df is sorted by lat, long
  below <- df[max(c(1, row_number - (n %/% 2))):row_number, ]
  above <- df[row_number:min(c((row_number + n / 2), nrow(df))), ]
  all = rbind(below, above)

  model <- lm(swing ~ change_query_incidence, data = all)
  coef(model)[["change_query_incidence"]]
}
...
with_corr <- with_lat_long %>%
    ...
    (\(df) df %>% mutate(local_slope = (1:nrow(df)) %>% map(\(row_num) get_local_slope(df, row_num))))
with_corr %>% select(change_query_incidence, swing, local_slope, lat, long) %>% head(10)
```
...
plot <- ggplot(data = map_data("state") %>% filter(long > -140)) +
  ...
  geom_point(data = with_corr %>% filter(long > -140), aes(x = long, y = lat, color = as.numeric(local_slope)), alpha = 1) +
  scale_color_continuous(low = "blue", high = "red") +
  labs(
    title = "Change in Search Interest for \"Immigration\" vs Electoral Swing from 2004 - 2012",
    color = "Local Correlation Between\nSearch Interest Change for\n\"Immigration\" and Election Results"
  ) +
  theme(text = element_text(size = 8))

ggsave("election_04_to_2012_heat_map_query_change_election_result.png")
...
swing_county <- votestimeframe %>%
    group_by(state, county_name, office) %>%
    summarise(swing = (.data[["candidatevotes_2020"]] / .data[["totalvotes_2016"]]) - (.data[["candidatevotes_2008"]] / .data[["totalvotes_2008"]])) %>%
    mutate(county = county_name) %>%
    select(county, swing)
...
with_corr.2 <- with_lat_long.2 %>%
    filter(!is.na(change_query_incidence) & !is.na(swing)) %>%
    (\(df) df %>% mutate(local_slope = (1:(nrow(df)) |> purrr::map(\(row_num) get_local_slope(df, row_num)))))
with_corr.2 %>% select(change_query_incidence, swing, local_slope, lat, long) %>% head(10)

plot <- ggplot(data = map_data("state") %>% filter(long > -140)) +
  geom_polygon(aes(x = long, y = lat, group = group), color = "white") +
  geom_point(data = with_corr.2 %>% filter(long > -140), aes(x = long, y = lat, color = as.numeric(local_slope)), alpha = 1) +
  scale_color_continuous(low = "blue", high = "red") +
  theme_minimal() +
  labs(
    title = "Change in Search Interest for \"Immigration\" vs Electoral Swing from 2008 - 2020",
    color = "Local Correlation Between\nSearch Interest Change for\n\"Immigration\" and Election Results"
  ) +
  theme(text = element_text(size = 8))
ggsave("election_08_to_2020_heat_map_query_change_election_result.png")
